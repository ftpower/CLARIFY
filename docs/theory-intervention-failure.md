...
## 15. 通信系统视角：LLM 幻觉的编码理论

> 本节将 LLM 推理形式化为一个通信问题，从检错编码、信道容量、分集接收、
> 和迭代译码的第一性原理出发，建立幻觉检测与干预的数学理论。
> 本框架是自成一体的——不依赖前文的实验结论，而是从通信理论推导出
> 可检验的设计原则。
...
**定义 2（层分解）.** 将信道在层 $\ell$ 处分裂为前半段（"干净段"）和后半段（"含噪段"）：

$$C_\theta = \underbrace{\text{RMSNorm} \circ W_U \circ f_L \circ \cdots \circ f_{\ell+1}}_{\text{含噪段 } C_{\ell:}^\theta} \circ \underbrace{f_\ell \circ \cdots \circ f_1 \circ \text{Embed}}_{\text{干净段 } C_{:\ell}^\theta}$$
...
LLM 推理中没有显式的生成矩阵 $G$ 和校验矩阵 $H$。但存在一个**隐式的奇偶校验结构**：模型参数 $\theta$ 定义了一个编码函数（"正确知识到表示"的映射），而不同 Transformer 层的输出可视为对同一码字的多次观测。
...
**关键性质**：由于 $y_{\ell_1}$ 和 $y_{\ell_2}$ 都是同一消息（$x$ 的正确输出）的编码，在没有"信道噪声"的理想情况下应有 $y_{\ell_1} = y_{\ell_2}$，因此 $S_{\ell_1, \ell_2} = 0$。非零校验子指示了"信道干扰"的存在。
...
在等协方差高斯假设下，此即 Fisher 线性判别方向，AUROC 衡量的是该方向分离两个分布的能力。

**关键洞察**：$v$ 的最优性只在**检测**意义上成立——它告诉我们"信号质量好不好"。但在**纠错**意义上，$v$ 不包含"如何修正"的信息——这不奇怪：奇偶校验矩阵 $H$ 能检错（$rH^T \neq 0$），但不能纠错（需要更复杂的译码算法）。

### 15.4 信道模型：级联子信道

#### 15.4.1 Transformer 层的马尔可夫结构
...
**性质 2（残差结构的分集增益）.** 残差连接 $h_\ell = h_{\ell-1} + \Delta_\ell$ 意味着信息可以在不衰减的情况下跨层传播。这是 LLM 中"信息保留"的物理基础，也是我们能在不同层观测到相关信号的物理原因。

#### 15.4.2 信道失真模型

对中间层 $\ell$ 到最终层 $L$ 的子信道 $C_{\ell:}^\theta$，定义其对"理想 logits"的失真：

**定义 6（信道失真算子）.** 子信道 $C_{\ell:}^\theta$ 对 logits 的效应为：

$$y_L = \Phi_\ell(y_\ell) + \eta_\ell(x)$$

其中 $\Phi_\ell: \mathbb{R}^{|\mathcal{V}|} \to \mathbb{R}^{|\mathcal{V}|}$ 是"合理推理"的非线性映射，$\eta_\ell(x)$ 是输入依赖的结构化失真（覆盖偏置）。

**关键假设（不可分离性假设）**：$\Phi_\ell$ 和 $\eta_\ell(x)$ 在 $y_L$ 中不可线性分离——即不存在矩阵 $P$ 使得 $P y_L$ 能完美分离 $\Phi$ 的贡献和 $\eta$ 的贡献。

**为什么这个假设重要**：如果 $\Phi$ 和 $\eta$ 可线性分离，那么用一个简单的线性投影就可以移除 $\eta$（反覆盖干预 14b 就会成功）。事实上 14b 的零结果构成了此假设的经验证据。
...
LLM 推理的"码"是隐式的——训练语料决定了"正确知识如何在参数空间中编码"。这个隐式码有三个致命问题：

1. **码率未知**: 知识在参数中以压缩形式存储，$k/n$ 不可知
2. **最小距离为零**: 两个不同问题的"正确表示"可能在隐藏空间的同一位置（表示冲突），这等价于 $d_{\min} = 0$
3. **错误模式不是二元的**: 错误是词表大小 $|\mathcal{V}| \approx 150$K 维 logit 空间中的任意偏移，而非 $\{0,1\}^n$ 中的比特翻转

**定理 1（线性分组码的无能性）.** 令 $v \in \mathbb{R}^d$ 为不依赖于 $x$ 的固定方向。定义干预 $h \leftarrow h + \alpha v$。则对任意 $v$，存在 logit 空间的错误模式 $e(x)$ 使得干预无法纠正，且这类错误模式在 $\mathcal{D}_{\text{KW}}$ 上的概率质量 > 0。

**证明概要**：固定方向 $v$ 在 logit 空间产生固定偏移 $J(x)v$。错误模式 $e(x)$ 是输入依赖的——两个不同问题 $x_1, x_2$ 需要不同的修正方向 $e(x_1) \neq e(x_2)$。固定的 logit 偏移只能"对"其中一个。$\square$

**推论**：这一定理不仅适用于我们的 $v$——它适用于**任何**不依赖于 $x$ 的固定干预方向。即使方向来自训练、来自对比学习、来自神谕梯度（作为固定方向的平均），结论不变。这个"不依赖于 $x$"才是关键约束。
...
其中 $\text{SNR}_k$ 衡量分集支路 $k$ 的信号质量。AUROC 可作为 SNR 的代理：

$$w_k \propto \frac{\text{AUROC}(\ell_k)}{1 - \text{AUROC}(\ell_k)}$$
...
**关键预测**：
- MRC 应优于 EGC（利用质量信息加权）
- MMSE 应优于 MRC（利用支路间相关性去冗余）
- 增加支路数 $K$ 的边际收益递减——高相关支路贡献小

#### 15.5.3 软判决译码

经典译码中，**硬判决**先对接收符号做二值化（0/1），然后译码。**软判决**保留接收信号的连续值，在译码时利用"置信度"信息——通常可获得 2-3 dB 的增益。

LLM 的硬判决是 $\arg\max$，等价于直接扔掉 logits 中的软信息。**TLDC 本质上是一种软判决译码**：在 logit 空间（软域）而非 token 空间（硬域）进行修正，然后才做硬判决。

**推论（软判决增益）**：在 logit 空间的修正（如 TLDC）应始终优于在 token 空间的修正（如对最终 argmax 做后处理），因为软信息在硬判决中丢失。

#### 15.5.4 迭代译码（Turbo/Belief Propagation 类比）

Turbo 码和 LDPC 码通过分量译码器之间交换软信息来实现逼近 Shannon 限的性能。核心机制是**外部信息的迭代交换**。

在 LLM 推理中，可以设计一个迭代译码过程：
...
这等价于在层之间传递"外部信息"——每个参考层提供关于信道噪声的一个独立估计，迭代合并这些估计逐步精炼 logits。

**关于收敛性**：由于 $S_{\text{compute}}$ 和 $S_{\text{override}}$ 不可分离（§15.4.2），过多次迭代会积累 $S_{\text{compute}}$ 的效应，导致性能退化。最优迭代次数 $T^*$ 应在独立校验子和合理推理损失之间取得平衡。
...
在 LLM 推理中，类比概念是 **"真值表达容量"**——给定输入 $x$，信道能多大程度上保留 model 已经"知道"的真相：
...
这是模型在无外部干预下正确表达已知知识的概率。Phase 16 诊断给出的估计：$C_{\text{truth}} \approx 45\%$。
...
我们的目标是设计 $D$ 使得 $C(\theta, \mathcal{D}, D) > C(\theta, \mathcal{D})$。

#### 15.6.2 译码器增益的上界

**定理 2（译码器增益上界）.** 不修改参数 $\theta$ 的推理时译码器的容量增益受限于信道本身的"软信息质量"：
...
**物理含义**：多抽头译码器最多能把那些"在某个参考层排第一，但在最终层被压制"的真相恢复出来。如果 $y_{\text{true}}$ 在所有参考层都不排第一，那么推理时译码器在信息论上无法纠正它——需要修改参数（训练）。

**推论**：此上界是样本相关的——它只对 margin（正确答案与第一名在 logit 空间的距离）足够小的样本允许纠错。margin 大的样本在推理时信息论不可纠。

#### 15.6.3 分集增益与层间距

**猜想 1（层间距与分集增益）.** 两个分集支路 $\ell_1, \ell_2$ 之间的相关性随层间距增大而减小：

$$\rho(\ell_1, \ell_2) \approx \exp(-\lambda \cdot |\ell_1 - \ell_2|)$$

其中 $\lambda > 0$ 是"信道相干系数"。

**引理 1（有效分集支路数的上界）.** 若猜想 1 成立，则有效独立分集支路数 $K_{\text{eff}}$ 有上界：

$$K_{\text{eff}} \leq \frac{L}{\ell_{\text{coh}}}, \quad \ell_{\text{coh}} = 1/\lambda$$

**设计准则**：选择间距 $\geq \ell_{\text{coh}}$ 的参考层来最大化分集增益。经验上 $\ell_{\text{coh}}$ 可由校验子相关矩阵（Gram 矩阵 $G$）的半衰距估计。
...
**性能**: 1.7B 模型上恢复 ~14% 的 KW 样本（单种子最优 28.6%，跨种子平均 14.3%）。

#### 15.7.2 乘积码（Multi-Layer TLDC）
...
传统通信中，**自适应调制编码 (AMC)** 根据信道质量动态调整编码速率和调制方式。

在 LLM 中，不同的输入 $x$ 遇到不同的"信道条件"——有些问题覆盖强，有些弱。因此 $\beta$ 应该是输入和 token 位置的函数：

**方案（信道质量指示 CQI）**: 用校验子的范数作为信道质量的代理：

$$\beta_t(x) = \beta_0 \cdot \frac{\|S_{\ell^*\to L}^{(t)}(x)\|}{\mathbb{E}[\|S_{\ell^*\to L}\|]}$$

其中 $t$ 是生成步索引。直觉：校验子范数大 → 信道在此 token 上的作用强（可能覆盖强，也可能合理推理多） → 更积极地纠错。

### 15.8 理论基础上的可检验假说

以下预测全部来自本节的通信理论框架，不依赖前 14 节的实验结论。

| # | 假说 | 理论出处 | 检验方法 |
|---|------|---------|---------|
| H1 | 检测方向 $v$ 是最优奇偶校验权重，但不是纠错方向 | §15.3.3 定理 1 | $v$ 的纠错效应 ≡ 0 (已由 10+ 实验确认) |
| H2 | 多抽头 MRC 分集合并 > 单抽头 EGC | §15.5.2, §15.7.2 | K 层 MRC vs K=1 TLDC, KW Δ 比较 |
| H3 | 支路间距 $|\ell_i - \ell_j|$ 越大，合并增益越大 | §15.6.3 猜想 1 + 引理 1 | {L5, L10, L20} vs {L18, L19, L20} |
| H4 | 软判决译码 (logit 空间) > 硬判决译码 (token 空间) | §15.5.3 | TLDC vs greedy decode with token reranking |
| H5 | 选取 AUROC 最高的 $K$ 层优于随机选 $K$ 层 | §15.5.2 MRC 权重 | MRC 用 top-K AUROC 层 vs 随机 K 层 |
| H6 | 存在最优迭代次数 $T^*$，超过后迭代译码退化 | §15.5.4 | Turbo 风格迭代; 扫描 $T=1,\ldots,5$ |
| H7 | 自适应 $\beta_t(x) \propto \|S(x)\|$ 优于固定 $\beta$ | §15.7.3 CQI 自适应 | 自适应 vs 固定 β |
| H8 | 有效独立分集支路数 $K_{\text{eff}}$ 远小于总层数 $L$ | §15.6.3 引理 1 | Gram 矩阵 $G$ 的有效秩 |
| H9 | 纠错容量与 logit margin $m(x)$ 分布的下尾概率成正比 | §15.6.2 定理 2 | 跨模型规模 margin 分布比较 |
| H10 | 单固定方向干预的容量为零——独立于方向来源 | §15.5.1 定理 1 | 任何不依赖 $x$ 的方向 $\Delta = 0$ |

### 15.9 开放问题与猜想

1. **信道互信息的层分解**: 能否量化每层对 $I(x; y_{\text{true}})$ 的边际贡献？如果某些层是净负贡献的（破坏互信息的），那么跳过这些层就能提高容量

2. **覆盖电路的可识别性**: 校验子 $S$ 中 $S_{\text{compute}}$ 和 $S_{\text{override}}$ 的比例能否通过对比不同输入来估计？如果有办法从外部估计覆盖分量，就能更精准地做减法

3. **容量的一般上界**: 对任意不修改 $\theta$ 的译码器 $D$，是否存在一个 Shannon 风格的上界 $C(\theta, \mathcal{D}, D) \leq C_{\max}(\theta, \mathcal{D})$？定理 2 给出了一个弱上界（依赖 rank=1 条件），但可能存在更紧的信息论上界

4. **编码-译码的联合设计**: 如果我们被允许在训练时对 $\theta$ 做**微小**修改（如添加低秩适配器），能否显式地为推理时译码器设计对偶的"码"？这类似于脏纸编码 (dirty paper coding)——发送方知道干扰存在时，可以预编码来减轻干扰

5. **跨模型规模的分集增益标度律**: $K_{\text{eff}}$ 是否随模型规模（层数）而增长？如果 $K_{\text{eff}} \propto L^{0.5}$，那么大模型天然具有更大的纠错潜力
