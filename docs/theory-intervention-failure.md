...
### 1.2.1 首 token 秩代理的保真度分析

> 2026-08-26 新增（理论先行；审计实验列入 `plans/current.md` 待办）。动机：§1.2 的"知道"代理用金标答案**首 token** $t_1$ 在最终层 logits 降序中的秩（1-indexed，修复后口径）判定，但首 token 未必携带答案信息——"The Eiffel Tower" 的首 token 是 "The"，知识在 "Eiffel"。

**问题形式化.** 设潜在知识状态 $K^* \in \{0,1\}$（答案信息是否在模型记忆中，不可直接观测），观测代理：

$$\hat{K} = \mathbb{1}\{\operatorname{rank}(t_1) \le 50\}.$$

代理误差由假知道率 $\alpha = P(\hat{K}=1 \mid K^*=0)$ 与假不知道率 $\beta = P(\hat{K}=0 \mid K^*=1)$ 刻画。

**机制假说（rank 的分解）.** 首 token 条件概率分解为先验与知识两部分：

$$\log P(t_1 \mid x) = \underbrace{\log P(t_1)}_{\text{语言模型流畅度（与知识无关）}} + \underbrace{\operatorname{PMI}(t_1; x)}_{\text{知识部分}}$$

- 冠词/介词/系动词等**功能词先验 $P(t_1)$ 极大**——任何流利英文模型里 "the"/"a"/"in" 的 rank 无条件靠前，与知识状态无关 → $\alpha$ 高（**假知道**：DK 被判为"知道"）
- 专名/数字先验低，rank 主要由 PMI 决定（信息量大）；但有子词切分风险（A.5 教训：首 token 编码与生成 token 必须一致）
- 推论：$\alpha$ 由答案首 token 的词性分布决定，**与模型规模/能力无关**——换 8B 不消除该偏差

**可检验预测.**

- P1（控制知识状态后）：功能词首 token 样本的 $\hat{K}=1$ 比例显著高于专名/数字首 token 样本
- P2：首 token 秩划分与序列级 logprob（长度归一化）划分的一致率 < 1，不一致样本集中于功能词首 token
- P3：假知道使观测"知道率"（KC+KW 占比）被高估：真实 DK 率 ≥ 观测 DK 率（TriviaQA 修复后观测 DK=51%）

**影响推论（分层）.**
- 检测 AUROC 的标签 = exact 对错判定，**不使用该代理** → 检测数字（truth direction 0.7564 / LR probe 0.7708）不受污染
- KW 子集 = 真 KW ∪ 假知道（DK 误入）→ TLDC 定案（已关闭）的目标人群含噪，影响限于"机制注脚"的表述强度
- rank 知识筛选实验（TriviaQA 无增益 0.7664≈0.7708）：筛选器本身带噪，"无增益"结论**可能被污染掩盖真实增益**，复验须换代理
- 开题报告 §3.1 的操作定义在审计完成前应注明代理误差

**保真条件与失效条件.**
- 保真：答案首 token 几乎总是信息性 token（纯数字答案、专名首 token 数据集）→ $\alpha \approx 0$
- 失效充分条件：功能词首 token 占比高，且知识状态与首 token 先验近似正交（开放问答大概率满足）

**缓解代理（理论排序，待审计定夺）.**
1. 序列级平均 logprob（长度归一化）——污染被稀释但残余
2. 信息量加权秩——取答案 token 中先验最低（最特异）者的秩，直接针对"关键字"直觉
3. 审计先行：量化 $\alpha$、$\beta$ 与划分一致率后再决定是否换代理
