...
## 下次优先（2026-08-31 下午继续）

1. **报告 Word 复核（用户侧，2026-09-10 答辩前）**：下午继续——① 封面制表位方案复核（13cm；想调长度可拖标尺或改脚本 TAB_POS_TWIPS，勿短于 11.5cm）② 替换封面测试值（导师/研究生/学号：王麻子/张三/25S123456）③ **Word 全部编辑完成后最后跑一次 `fix_math_font.py` 收尾**（中途保存即重置公式字体）④ 第 8 章重写版、2.3 压缩版确认 ⑤ 封面占位符 + 1.1 导师信息 + Word 更新 TOC；图 3.3/5.1、段落 113、第 1/2 章扩写、括号清理（153→131）待复核；参考文献 [1] arXiv id、[13] venue 终核
2. **实验侧（可选，不阻塞开题）**：① Phase 24 β sweep 修复后重跑（P0 最后一项，唯一未重跑的头部数字；重跑后需更新图 5.7/5.8 与正文数字）② SNR/LLR 探索阶段 1（噪声定义 A/B 待定）③ 8B rank 筛选验证（服务器可选补项）
3. **git**：本次 session-end sync 已 commit，用户 push（当前无未 push commit）
