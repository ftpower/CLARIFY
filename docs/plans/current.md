...
- **新增（2026-08-25，论文关键）：旧干预范式代码复核**——开题 5.2 节只写 TLDC 弱效应；Phase 4/7/9/11-16 的零效应结论（ITI/RepE/ROME/子空间/几何感知/FactCheckmate/内部稽查等 ~17 脚本）须逐一按统一清单复核后方可写入论文。清单：①截断保尾部 ②exact 标签 ③held-out/CV ④rank 1-indexed ⑤干预 l_final 用模型真实 logits（lens 重算=今日伪影）⑥参数不得在测试集选 ⑦.detach() 断梯度 ⑧无 max(auroc,1-auroc) 符号翻转
