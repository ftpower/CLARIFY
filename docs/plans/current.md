...
## 下次优先（2026-09-02 第 5 章重排 + TOC 更新 + 文件替换后）

1. **报告 Word 复核（用户侧，2026-09-10 答辩前）**：
   - ✅ **已完成**：TOC 更新域（用户 9/2 Word 更新，第 3/5 章新结构条目均刷新）；全文交叉引用审计（0 命中，2026-09-01）；第 3 章重构版 + 第 5 章重排版（5.1 理论准备前置）复核通过（渲染 41 页）；公式字体收尾（fix_math_font 9/2 跑完：229 处显式 Cambria→TNR + 83 处正文，0 残留）
   - ⏳ **剩余（用户侧）**：① 封面测试值（王麻子/张三/25S123456）② 第 8 章/2.3 确认、1.1 导师信息 ③ 参考文献 [1] arXiv id、[13] venue 终核 ④ ⚠️ **若还要在 Word 编辑并保存，最后重跑一次 `fix_math_font.py`**（Word 保存即重置公式字体；脚本已前缀无关 + 备份名递增）
2. **实验侧（可选，不阻塞开题）**：① Phase 24 β sweep 修复后重跑（P0 最后一项；重跑后更新图 5.7/5.8 与正文数字）② SNR/LLR 探索阶段 1（噪声定义 A/B 待定）③ 8B rank 筛选验证（服务器可选补项）
3. **git**：本场 session-end sync 修改 `docs/plans/current.md` + `docs/project-state.md`，**待用户 push**（未推：7bd98dc、5c85a93 及更早）
