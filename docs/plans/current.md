...
**3. 遗留（用户 Word 侧）**：① TOC 更新域（第 3 章重构 + 第 5 章重排后目录刷新）② 封面测试值（王麻子/张三/25S123456）③ 第 8 章/2.3 确认、1.1 导师信息 ④ 参考文献 [1] arXiv id、[13] venue 终核 ⑤ **公式字体收尾：Word 全部编辑完成后最后跑一次 `fix_math_font.py`**（用户 9/1 15:38 Word 保存已把 182 个公式 run 的 rFonts 清空，当前文件待修复）
