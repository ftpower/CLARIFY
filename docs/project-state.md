...
| 干预：Δ accuracy > 0，统计显著，跨模型/数据集泛化 | ❌ **未达成**（10+ 范式零效应，信息论上限已触及；TLDC **大样本定案 2026-08-25**：KW 效应真实——双 seed 方向一致 + pooled CI 下界 β≥0.03 起 >0——但代价大，无单一 β 同时满足「双 seed KW CI>0 + KC<5%」→ 中间态偏有效，per-token 机制分析定案「不对称惩罚」存废） |
...
2. **TLDC per-token 机制分析（2026-08-25 进行中）**：大样本定案完成（效应真实但昂贵，中间态偏有效）；per-token 分析（analyze_tldc_per_token.py 已修三 bug + KC/DK 组统计）正在跑（双 seed β=0.03）——回答「不对称惩罚在完整 prompt 下是否成立」与「seed 异质来源」，是 TLDC 存废的真正判据
...
- **gate 证伪要对照机制分析确认前提**（2026-08-25）：D2 检验的是"rank 恢复"旧假说，而机制分析（theory §4）已证明 TLDC 是"不对称惩罚"——用 D2 证伪关闭 TLDC 是把旧假说当成了成立条件
