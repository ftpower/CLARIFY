...
| 干预：Δ accuracy > 0，统计显著，跨模型/数据集泛化 | ❌ **未达成**（10+ 范式零效应，信息论上限已触及；TLDC **重审中**：D2 证伪只否「rank 恢复」假说，弱正信号 2/24（CI [1%, 27%]）待大样本定案；定理 2 上界收紧 ≈0） |
...
2. **TLDC 大样本定案（2026-08-25 新增）**：n_test 300-500（KW ≥ 75-100）+ β 覆盖 {0.01, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20} + 双 seed 复现；修复后重跑 per-token 机制分析（不对称惩罚是否在完整 prompt 下成立）——见 code-review-2026-08-24.md 重审节
