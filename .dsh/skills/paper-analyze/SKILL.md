...
本 skill 默认使用 `pytorch_env0` conda 环境。所有 `python3` 命令都应在该环境下执行。

```bash
# 激活环境（如需要）
# conda activate pytorch_env0
# Python 路径
PYTHON=/home/user_ft/miniconda3/envs/pytorch_env0/bin/python
```

> 使用 `$PYTHON` 代替 `python3` 执行所有 Python 脚本。
...
### 0.1 如果传入的是本地 PDF 文件路径，自动提取 arXiv ID
...
> 提取到的 arXiv ID 将用于后续的 API 查询和脚本调用。

### 0.2 如果传入的是 arXiv ID 或标题，直接使用
...
5. **公式变量必须逐一解释（重要！）**
   - 引入任何公式后，必须对公式中**每个变量**同时给出：
     - **数学定义**：该符号在公式中是什么（如"后验分布"、"经验风险"、"置信参数"）
     - **实际含义**：该变量在论文语境中代表什么（如"训练后实际得到的模型分布"、"训练集上的平均幻觉率"）
   - 格式示例：
     ```
     $$R_{\text{hall}}(M) \leq \hat{R}_{\text{hall}}(M) + \sqrt{\frac{\text{KL}(Q\|P) + \log\frac{2\sqrt{n}}{\delta}}{2n}}$$

     其中：
     - $\hat{R}_{\text{hall}}(M)$：经验幻觉率，即在训练集上实际测得的幻觉比例
     - $Q$：后验分布，训练后实际得到的模型分布（确定性模型下退化为点质量）
     - $P$：先验分布，看到数据前对模型空间的初始信念
     - $\text{KL}(Q\|P)$：模型复杂度惩罚，Q 与 P 偏离越大则泛化界越松
     - $n$：训练样本数
     - $\delta$：置信参数，界以概率 $1-\delta$ 成立
     ```
   - **禁止**仅写"其中 X 是 Y"而不解释 Y 的实际含义
   - 如果原文未明确定义某个变量，需根据上下文推断并标注"原文未显式定义，推断为..."
...
# 1. 提取所有嵌入原图（论文的 figure）
for i in range(len(doc)):
    for j, img in enumerate(doc[i].get_images()):
        xref = img[0]
        base = doc.extract_image(xref)
        fname = f'{img_dir}/fig{j+1}_raw.{base["ext"]}'
        with open(fname, 'wb') as f:
            f.write(base['image'])

# 2. 对症结页面做裁剪渲染（表格页、架构图等）
def render(page_num, name, zoom=2.0, top=55, bottom=50):
    page = doc[page_num - 1]
    clip = fitz.Rect(0, top, page.rect.width, page.rect.height - bottom)
    pix = page.get_pixmap(matrix=fitz.Matrix(zoom, zoom), clip=clip)
    pix.save(f'{img_dir}/{name}')
PYEOF
```

### 3.2 语义重命名（强制步骤）

**禁止保留原始页号命名**。每张图必须根据论文中实际内容重命名：

```bash
# ❌ 禁止
page8_fig1.png  page10_fig3.png  page12_fig45.png

# ✅ 必须
fig1_selectivity.png        # 论文 Figure 1
fig2_percentile_sweep.png   # 论文 Figure 2
fig3_ablation.png           # 论文 Figure 3
table1_contributions.png    # 关键表格页渲染
```

文件名格式：`{论文原编号}_{内容简述}.{ext}`

### 3.3 生成 images/index.md（强制步骤）

**必须**按以下格式生成图片索引，按类别分组：

```markdown
# 图片索引 - [论文简称]

## 核心结果
- `fig1_xxx.png` - Figure 1: [描述]
- `fig2_xxx.png` - Figure 2: [描述]

## 轨迹/机制分析
- `fig5_xxx.png` - Figure 5: [描述]

## 消融与对比
- `fig7_xxx.png` - Figure 7: [描述]
```

### 3.4 清理无用图片

删除空白页渲染、纯参考文献页渲染等无用图片。
...
**场景 A：本地 PDF 文件**（从文件路径传入）
- 笔记和图片必须放在 **PDF 所在目录**，不根据领域新建文件夹
- 从 PDF 内容推断领域（记录在 frontmatter 和 graph 中），但不按领域分目录

```bash
PDF_DIR=$(dirname "[PDF文件路径]")
PAPER_TITLE="[论文标题，空格替换为下划线]"
NOTE_PATH="${PDF_DIR}/${PAPER_TITLE}.md"
IMAGES_DIR="${PDF_DIR}/${PAPER_TITLE}/images"
INDEX_PATH="${IMAGES_DIR}/index.md"
```

**场景 B：从 arXiv 直接分析**（传入 arXiv ID，无本地 PDF）
- 按领域分目录存放
...
领域推断规则：
- 提到"agent/swarm/multi-agent/orchestration" → 智能体
- 提到"vision/visual/image/video" → 多模态技术
- 提到"reinforcement learning/RL" → 强化学习_LLM_Agent
- 提到"language model/LLM/MoE" → 大模型
- 否则 → 其他
...
在报告完成前，必须逐项确认"质量检查清单"中的 8 项全部通过：
1. 图片已语义命名（非 pageN_xxx.png）
2. images/index.md 已生成
3. 无用图片已清理
4. 机构信息已提取
5. 已搜索并对比 vault 中相关论文
6. 公式变量已逐一解释
7. 实验数据以表格呈现
8. 所有交付物文件已就位

**如任一项未通过，不得报告完成，必须先修复。**
...
## 质量检查清单（每次分析必须逐项确认，禁止跳过）

**在报告分析完成之前，必须逐项自检以下清单。这是强制步骤，不得省略任何一项。**

### 图片处理（最容易出问题的环节）

1. **图片语义命名**（禁止原始页号命名）
   - ❌ 禁止：`page8_fig1.png`、`page10_fig3.png`、`page12_fig2.png`
   - ✅ 必须：`fig1_selectivity.png`、`fig2_percentile_sweep.png`、`fig3_ablation.png`
   - 规则：每张图必须根据其实际内容命名，格式为 `fig{N}_{内容描述}.{ext}`
   - 如果提取出的图是嵌入原图（非页面渲染），直接使用语义命名
   - 页面渲染图作为补充时，同样使用语义命名而非 `page{N}_xxx.png`

2. **index.md 强制生成**
   - 每篇论文的 images/ 目录下 **必须** 有 `index.md`
   - index.md 按类别组织所有图片（如：核心结果、轨迹分析、消融与对比、机制分析）
   - 每张图一行：`- fig{N}_{描述}.png - 图的简要说明`
   - index.md 是图片索引，不是记忆文件——只描述图片内容

3. **图片清理**
   - 分析完成后删除无用图片（重复渲染、空白页、只含参考文献的页面渲染等）
   - 保留论文中的所有原图及关键表格所在页面的渲染

### 笔记内容检查

4. **机构信息**
   - 必须提取作者机构（从 PDF 首页 `\author`/`\affiliation` 字段或作者脚注）
   - 机构字段不能为空或 `--`（除非 PDF 确实不含此信息）
   - 即使 arXiv API 不可用，也要从 PDF 文本中手动提取

5. **与已分析论文对比**
   - 必须在 vault 中搜索是否已有同领域论文的分析笔记
   - 如果存在相关论文，必须在"与相关论文对比"章节中加入方法对比表格
   - 对比维度包括：目标、理论基础、操作对象、访问需求、实时性等
   - 不能只列论文名而不做实际对比

6. **公式变量解释**
   - 每个公式后必须逐一解释每个变量的数学定义和实际含义
   - 禁止仅写"其中 X 是 Y"而不解释 Y 的实际含义

7. **实验数据完整性**
   - 主实验结果必须包含具体数值的表格，不能用文字概括代替
   - 如果论文有跨模型/跨方法的对比表，必须完整呈现核心数据

### 交付物检查

8. **文件清单确认**
   - 本地 PDF：`[PDF同级目录]/[论文标题].md` + `[PDF同级目录]/[论文标题]/images/`
   - arXiv 来源：`[领域]/[论文标题].md` + `[领域]/[论文标题]/images/`
   - `[论文标题]/images/index.md` — 图片索引
   - `graph/graph_data.json` — 已更新节点和边

## 重要规则

- **保留用户现有笔记** - 不要覆盖手动笔记
- **本地 PDF 笔记路径** - 笔记和图片放在 PDF 所在目录，禁止根据领域新建文件夹（arXiv 来源则按领域分目录）
...
- **每次分析必须通过质量检查清单** - 上述8项全部确认后才能报告完成
...
4. **机构/Affiliation 提取**：从 PDF 文本提取作者机构和脚注；这是强制项，不可标 `--` 除非 PDF 确实不含此信息
